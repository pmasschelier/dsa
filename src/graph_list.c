...
static int add_edge_list_noverif(GRAPH_LIST *g, unsigned int a, unsigned int b,
                                 long long weight) {
...
void set_edge_list(GRAPH_LIST *g, unsigned int a, unsigned int b, BOOL val,
                   long long weight, BOOL reverse) {
  list_ref_t *neighbours = &g->neighbours[a];
...
static int allow_tab_father_mark(unsigned length, int **tab, int **father,
                                 char **mark) {
  if (tab) {
...
int mark_and_examine_traversal_list(GRAPH_LIST *g, unsigned r, int **tab,
                                    int **father, LIST_STRUCT queue_or_stack) {
  char *mark;
...
static void DFS_list_recursive_rec(GRAPH_LIST *g, unsigned current, int **tab,
                                   int **father, char *mark, unsigned *index) {
  mark[current] = 1;
  (*tab)[*index] = current;
  *index += 1;
  node_ref_t *node = g->neighbours[current].begin;
...
static unsigned father_heap(unsigned i) { return i == 0 ? 0 : (i + 1) / 2 - 1; }

static unsigned ls_heap(unsigned i) { return (i + 1) * 2 - 1; }

static unsigned rs_heap(unsigned i) { return (i + 1) * 2; }
...
static int allow_father_index_distance_mark(unsigned length, int **father,
                                            unsigned **index,
                                            long long **distance, BOOL **mark) {
  if (father) {
    *father = malloc(sizeof(int[length]));
    if (!*father)
      return -1;
...
int Dijkstra_list(GRAPH_LIST *g, unsigned r, long long **distance,
                  int **father) {
  if (!distance)
    return -1;
  BOOL *mark;
  ...
  while (size >
    ...
    while (node != NULL) { // Pour tout successeur de pivot
      ...
      int d = (*distance)[pivot] > INFINITY - e->w ? INFINITY
                                                   : (*distance)[pivot] + e->w;
      if (d < (*distance)[e->p]) {
        (*distance)[e->p] = d;
        if (father)
