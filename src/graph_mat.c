...
void set_edge_mat(GRAPH_MAT *g, unsigned int a, unsigned int b, BOOL val,
...
int mark_and_examine_traversal_mat(GRAPH_MAT *g, unsigned r, int **tab,
...
static void DFS_mat_recursive_rec(GRAPH_MAT *g, unsigned current, int **tab,
...
static int allow_father_distance_mark(unsigned length, int **father,
...
int Dijkstra_mat(GRAPH_MAT *g, unsigned r, long long **distance, int **father) {
  ...
  for (unsigned i = 0; i < g->nb_vert - 1; i++) {
    for (unsigned j = 0; j < g->nb_vert; j++) { // Pour tout sommet j
      if (!mark[j] &&
          g->mat[pivot][j].b) { // non encore dans A et successeur de pivot
        const long long d = (*distance)[pivot] > INFINITY - g->mat[pivot][j].w
                                ? INFINITY
