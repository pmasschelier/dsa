...
btree_node_t* create_btree_leaf(
	void* value,
	btree_node_t* parent,
    uintptr_t priv_init,
    size_t size_bytes)
{
	btree_node_t* leaf = malloc(sizeof(btree_node_t) + size_bytes);
...
void btree_swap_node(btree_node_t*** a, btree_node_t*** b) {
    btree_node_t* a_ptr = **a;
    btree_node_t* b_ptr = **b;
    btree_node_t a_val = *a_ptr;
    btree_node_t b_val = *b_ptr;
...
static unsigned btree_height_rec(node_btree_t* node, unsigned height) {
	if (node)
...
unsigned btree_height(btree_t* tree) {
...
unsigned btree_length(btree_t* tree) {
	return btree_preorder_traversal(tree, NULL);
}

btree_node_t** btree_next_node(btree_node_t* node, btree_path_t* p) {
	return path_walk_forward(p, &node->ls, &node->rs);
}

#ifdef STRUCT_RECURSIVE_IMPL
static node_btree_t* btree_emplace_at_rec(node_btree_t** node_ptr,
                                              node_btree_t* parent,
											  btree_path_t path,
	...
	if (*node_ptr == NULL)
		return NULL;
	node_btree_t** son = btree_next_node(*node_ptr, &path);
...
#else
btree_node_t *btree_emplace_at(btree_t *tree, btree_path_t path,
                                   void *p) {
    btree_node_t **node_ptr = &tree->root;
...
#ifdef STRUCT_RECURSIVE_IMPL
static int btree_emplace_path_rec(node_btree_t** node_ptr,
								  btree_path_t path,
								  void* values[],
								  int index,
								  size_t length,
	...
	if (*node_ptr == NULL) {
		*node_ptr = malloc(sizeof(node_btree_t));
...
int btree_emplace_path(btree_t* tree,
					   btree_path_t path,
					   void* values[],
					   size_t length,
					   size_t offset) {
...
#ifdef STRUCT_RECURSIVE_IMPL
void btree_preorder_traversal_rec(node_btree_t* node, lambda_t* lambda, unsigned* i) {
    if(node == NULL)
        return;
...
int btree_preorder_traversal(btree_t* tree, lambda_t* lambda) {
	unsigned i = 0;
    btree_preorder_traversal_rec(tree->root, lambda, &i);
    return i;
}

static void btree_preorder_traversal_array_rec(node_btree_t* node,
										 void* tab[],
										 unsigned* i) {
	if (node == NULL)
        return;
...
int btree_preorder_traversal_array(btree_t* tree, void* tab[]) {
	unsigned i = 0;
	btree_preorder_traversal_array_rec(tree->root, tab, &i);
	return i;
}

int btree_dfs_array_rec(node_btree_t* node,
...
int btree_dfs_array(btree_t* tree,
			  void* preorder[],
			  void* inorder[],
			  void* postorder[]) {
...
int btree_dfs_rec(node_btree_t* node,
			  lambda_t* preorder,
			  lambda_t* inorder,
			  lambda_t* postorder) {
    if(node == NULL)
...
enum dfs_status { PREORDER, INORDER, POSTORDER };

#define btree_foreach(tree) \
    for(node_btree_t)
...
int btree_dfs(btree_t* tree,
	...
	do {
		...
		case UPWARD_RIGHT:
			...
			btree_node_t* next = stack_view_peak(forest, btree_node_t);
			if (next == NULL)
				goto exit;
		...
		node = stack_view_peak(forest, btree_node_t);
	} while (node != NULL);
exit:
	free_stack(forest);
    return count;
...
/* int btree_preorder_traversal(btree_t* tree, void* tab[]) { */
/* 	unsigned i = 0; */
/* 	btree_dfs(tree, node, tab[i++] = node->p, , ); */
/* 	return i; */
/* } */

/* int btree_postorder_traversal(btree_t* tree, void* tab[]) { */
/* 	unsigned i = 0; */
/* 	btree_dfs(tree, node, , , tab[i++] = node->p); */
/* 	return i; */
/* } */

/* int btree_inorder_traversal(btree_t* tree, void* tab[]) { */
/* 	unsigned i = 0; */
/* 	btree_dfs(tree, node, , tab[i++] = node->p, ); */
/* 	return i; */
/* } */
...
#ifdef STRUCT_RECURSIVE_IMPL
static void btree_clean_rec(node_btree_t* node) {
	if (node->ls)
...
void btree_clean(btree_t* tree) {
	...
	if (tree->root == NULL)
		return;
	stack_view_t* stack = create_stack_view(sizeof(node_btree_t*));
	when_null_ret(stack, );
...
int btree_levelorder_traversal(btree_t* tree, void* tab) {
	when_null_ret(tree, -ERROR_INVALID_PARAM1);
	if (tree->root == NULL)
		return 0;
	unsigned i = 0;
    
	linked_list_t forest = LINKED_LIST_INIT(btree_node_t*);
...
btree_t* btree_perfect_tree_from_tab(void* tab,
										 size_t size,
										 unsigned length) {
